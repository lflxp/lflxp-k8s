  ...
  const handleUpdate = () => {
    try {
      ...
      .then(_data => {
        ...
        toast({
          title: "刪除成功",
          description: `Daemonset ${currentRow.name} 配置已刪除`,
        });
      })
      .catch(_error => {
        toast({
      ...
      {currentRow && (
        <>
          ...
          <ConfirmDialog
            ...
            namespace={currentRow.namespace}
            className='max-w-md'
            title={`Delete DaemonSets: ${currentRow.namespace}/${currentRow.name} ?`}
            desc={
              <>
                You are about to delete a DaemonSets with the Name{' '}
